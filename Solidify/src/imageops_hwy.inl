...
namespace HWY_NAMESPACE {
    namespace {
        ...
        template<class V> HWY_ATTR V selectRgb(const V r, const V g, const V b, const uint8_t channel)
        {
            switch (channel) {
            case 1: return g;
            case 2: return b;
        ...
        template<typename T> HWY_ATTR T invertScalar(const T v, const bool signedRange)
        {
            if constexpr (std::is_floating_point_v<T>) {
        ...
        template<>
        HWY_ATTR void loadRgbAlpha<float>(const float* HWY_RESTRICT src, const int nchannels,
                                          const hn::ScalableTag<float> d, hn::VFromD<hn::ScalableTag<float>>& r,
                                          hn::VFromD<hn::ScalableTag<float>>& g, hn::VFromD<hn::ScalableTag<float>>& b,
        ...
        template<>
        HWY_ATTR void
        loadRgbAlpha<double>(const double* HWY_RESTRICT src, const int nchannels, const hn::ScalableTag<double> d,
                             hn::VFromD<hn::ScalableTag<double>>& r, hn::VFromD<hn::ScalableTag<double>>& g,
                             hn::VFromD<hn::ScalableTag<double>>& b, hn::VFromD<hn::ScalableTag<double>>& a)
        {
            if (nchannels == 4) {
                hn::LoadInterleaved4(d, src, r, g, b, a);
            } else {
        ...
        HWY_ATTR bool swapInvertTyped(const SolidifyHwyImageView* view, const SolidifyHwySwapOp* op)
        {
            ...
            for (int y = 0; y < view->height; ++y) {
                const uint8_t* srcBytes = static_cast<const uint8_t*>(view->src)
                                          + static_cast<size_t>(y) * view->srcRowStride;
        ...
        HWY_ATTR hn::VFromD<hn::ScalableTag<T>>
        grayFixed(const hn::ScalableTag<T> d, const hn::VFromD<hn::ScalableTag<T>> r,
                  const hn::VFromD<hn::ScalableTag<T>> g, const hn::VFromD<hn::ScalableTag<T>> b,
        ...
        template<typename T>
        HWY_ATTR hn::VFromD<hn::ScalableTag<T>>
        grayFloat(const hn::ScalableTag<T> d, const hn::VFromD<hn::ScalableTag<T>> r,
                  const hn::VFromD<hn::ScalableTag<T>> g, const hn::VFromD<hn::ScalableTag<T>> b,
                  const SolidifyHwyGrayscaleOp* op)
        {
                ...
                for (; x < view->width; ++x) {
                    ...
                    case 7:
                        if constexpr (std::is_floating_point_v<T>) {
                            ...
                            const unsigned long long maxValue = static_cast<unsigned long long>(
        ...
        {
            if ((view->srcChannels != 3 && view->srcChannels != 4)
                || (view->dstChannels != 1 && view->dstChannels != 2)) {
                return false;
            }
