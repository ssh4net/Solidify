...
static bool
runSwapInvertHwy(const SolidifyHwyImageView* view, const SolidifyHwySwapOp* op)
{
    return HWY_DYNAMIC_DISPATCH(SwapInvertKernel)(view, op);
}

static bool
runGrayscaleHwy(const SolidifyHwyImageView* view, const SolidifyHwyGrayscaleOp* op)
{
...
static int
pixelTypeFromFormat(const OIIO::TypeDesc format)
{
...
static bool
canUsePackedHwy(const OIIO::ImageBuf& src, const OIIO::ImageBuf& dst)
{
    ...
    return src.localpixels() != nullptr && dst.localpixels() != nullptr && sspec.depth == 1 && dspec.depth == 1
           && sspec.format == dspec.format
           && pixelTypeFromFormat(sspec.format) != solidify_hwy::SolidifyHwyPixelType_Unsupported
...
static bool
runSwapInvertHwyParallel(OIIO::ImageBuf& dst, const OIIO::ImageBuf& src, const solidify_hwy::SolidifyHwySwapOp& op,
                         const int nthreads)
{
    ...
    OIIO::ROI roi(src.xbegin(), src.xend(), src.ybegin(), src.yend(), src.zbegin(), src.zend(), 0, src.nchannels());
...
static bool
runGrayscaleHwyParallel(OIIO::ImageBuf& dst, const OIIO::ImageBuf& src, const solidify_hwy::SolidifyHwyGrayscaleOp& op,
                        const int nthreads)
{
    if (!canUsePackedHwy(src, dst)) {
        return false;
...
static void
setFixedWeights(solidify_hwy::SolidifyHwyGrayscaleOp* op)
{
    if (op->mode == 6) {
...
static void
setGraySpec(OIIO::ImageSpec* spec, const int outChannels)
{
...
applyChannelSwapInvert(OIIO::ImageBuf& dst, const OIIO::ImageBuf& src, unsigned int basis, unsigned int invertMask,
                       const bool signedRange, const int nthreads)
{
    if (!src.initialized()) {
